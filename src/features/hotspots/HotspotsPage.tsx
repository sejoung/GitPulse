...
export function HotspotsPage() {
  ...
  const { data: hotspotRows = [], isLoading } = useHotspotsAnalysis(workspacePath, selectedBranch, analysisPeriod, excludedPaths, bugKeywords);
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        <StatCard label={t("stats.topChurnFiles")} value={!hasWorkspace ? t("common:status.notAnalyzed") : isLoading ? "..." : String(hotspotRows.length)} detail={t("common:time.lastYear")} tone={hasWorkspace ? "brand" : "neutral"} />
        <StatCard label={t("stats.bugOverlap")} value={!hasWorkspace ? t("common:status.notAnalyzed") : isLoading ? "..." : String(bugOverlap)} detail={t("stats.highChangeAndFixHeavy")} tone={hasWorkspace ? "watch" : "neutral"} />
        <StatCard
          label={t("stats.highestSignal")}
          value={!hasWorkspace ? t("common:status.notAnalyzed") : isLoading ? "..." : hasData ? t(`common:status.${highestSignal}`) : t("common:empty.hotspots")}
      ...
      <ChartCard title={t("chart.title")} description={t("chart.description")}>
        {hotspotRows.length === 0 ? (
          <EmptyState title={hasWorkspace ? t("common:empty.hotspots") : t("common:empty.selectWorkspace")} />
